...
- **`final` — это зафиксированная строка.** Обогащается на границе
  ...
  пунктуирует сама — см. [Модели и бэкенды](07-models-and-backends.md).
...
| `--vad` | **только Silero VAD** — blank-run игнорируется | `--vad-min-silence-ms` (дефолт 500) |
...
`--vad`). Непрерывная речь без пауз тоже финализируется — стриминговое окно
упирается в ~2.5 с и сдвигается. `final` также приходит по `stop` (рецепт 3)
и перед закрытием по инициативе сервера (рецепт 4).
...
- В этой книге: [Начало работы](01-getting-started.md) — установка и первый
  запуск, [Модели и бэкенды](07-models-and-backends.md) — головы и поведение
