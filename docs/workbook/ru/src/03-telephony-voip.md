...
Общее для всех путей: по умолчанию ограничения на длительность нет — длинные
звонки транскрибируются штатно, а `?codec=` / `?sample_rate=` одинаково
работают на `/v1/transcribe`, `/v1/transcribe/stream` и `/v1/jobs`. Тем, кому
нужен явный лимит, доступен `--max-audio-secs` при запуске сервера (см.
ошибку ниже); пути, держащие весь буфер целиком (VAD, диаризация,
`channels=split`, декодирование телефонии/Opus), всё равно ограничены
фиксированным потолком ~30 минут. Deepgram-совместимый эндпоинт `/v1/listen`,
принимающий те же телефонные форматы, в работе.
...
- **«Audio file too long»** (`audio_too_long`, HTTP 413). По умолчанию лимита
  длительности нет, поэтому ошибка появляется только если сервер запущен с
  явным `--max-audio-secs <N>`, либо загрузка упирается в фиксированный
  потолок ~30 минут на пути, держащем весь буфер целиком, — VAD, диаризация,
  `channels=split` или декодирование телефонии/Opus. На основном пути делить
  запись по длине больше не нужно; на путях с целым буфером делите запись по
  плечам звонка (или отключите соответствующую функцию для этого файла).
