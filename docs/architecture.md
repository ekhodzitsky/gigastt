...
Embed inference in any Rust project with `gigastt-core = "2.11"`. For a lean embedded build, disable defaults (`default-features = false`) to drop `tokio` / `reqwest` / `symphonia`; opt capabilities back in via the `net`, `async-pool`, and `file-decode` features.
