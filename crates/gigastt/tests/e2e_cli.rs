...
// ─── telephony codecs: G.722 WAV + raw streams via --codec ──────────────────

/// Telephony transcodes of the golos_00 speech fixture ("шестьдесят тысяч
/// тенге сколько будет стоить"), generated by
/// scripts/generate_telephony_fixtures.sh.
const TELEPHONY_DIR: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/fixtures/telephony");

/// `transcribe --codec` without `--sample-rate` fails at argument parsing,
/// before any model load — so this test needs no model.
#[test]
fn cli_transcribe_codec_without_sample_rate_fails() {
    let out = Command::new(bin())
        .args([
            "transcribe",
            "call.ulaw",
    ...
    assert!(
        !out.status.success(),
        "--codec without --sample-rate must fail at parse time"
    );
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
...
fn cli_transcribe_g722_wav() {
    ...
    let out = Command::new(bin())
        .args([
            "transcribe",
            &format!("{TELEPHONY_DIR}/speech_g722.wav"),
            "--model-dir",
            &md,
            "--punctuation",
            "off",
    ...
    assert!(
        out.status.success(),
        "G.722 WAV transcribe failed: {}",
        String::from_utf8_lossy(&out.stderr)
    );
    let stdout = String::from_utf8_lossy(&out.stdout).to_lowercase();
    assert!(
        ["шестьдесят", "тенге", "стоить"]
            .iter()
            .any(|kw| stdout.contains(kw)),
        "G.722 WAV transcript must contain a reference keyword, got: {stdout}"
    );
}
...
fn cli_transcribe_raw_ulaw_with_codec_flags() {
    ...
    let out = Command::new(bin())
        .args([
            "transcribe",
            &format!("{TELEPHONY_DIR}/speech.ulaw"),
            "--codec",
            "pcmu",
            "--sample-rate",
            "8000",
    ...
    assert!(
        ["шестьдесят", "тенге", "стоить"]
            .iter()
            .any(|kw| stdout.contains(kw)),
        "raw μ-law transcript must contain a reference keyword, got: {stdout}"
    );
}

// ─── Opus: OGG/Opus and .opus files via the opus-rs fallback decoder ───────

/// Opus transcodes of the golos_00 speech fixture ("шестьдесят тысяч
/// тенге сколько будет стоить"), generated by
    ...
    for fixture in ["speech_16k.ogg", "speech_telegram.ogg", "speech.opus"] {
        ...
        assert!(
            ["шестьдесят", "тенге", "стоить"]
                .iter()
                .any(|kw| stdout.contains(kw)),
            "{fixture} transcript must contain a reference keyword, got: {stdout}"
        );
    }
}
