...
// Opus: OGG/Opus and .opus containers via the pure-Rust opus-rs decoder
// ---------------------------------------------------------------------------

/// Directory with the Opus transcodes of the golos_00 speech fixture
/// ("шестьдесят тысяч тенге сколько будет стоить"), generated by
/// scripts/generate_opus_fixtures.sh.
const OPUS_DIR: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/fixtures/opus");

/// Decode-level smoke test for the Opus fixtures (no model needed, runs on
/// every PR): every container must decode through the generic bytes path to
/// ≈4 s of 16 kHz audio. Symphonia demuxes the OGG container; the packets
/// are decoded by the pure-Rust opus-rs fallback.
#[test]
fn test_opus_fixtures_decode_without_model() {
    // 4 s fixture at 16 kHz = 64_000 samples; the untrimmed pre-skip and the
    // resampler's FIR delay shift the count by a few hundred.
    for fixture in ["speech_16k.ogg", "speech_telegram.ogg", "speech.opus"] {
        let bytes = std::fs::read(format!("{OPUS_DIR}/{fixture}")).expect("read fixture");
        let samples = gigastt::inference::audio::decode_audio_bytes(&bytes)
            .unwrap_or_else(|e| panic!("{fixture} must decode: {e:#}"));
        assert!(
            samples.len() >= 60_000,
            "{fixture}: expected ≥60_000 samples, got {}",
            samples.len()
        );
        assert!(
            samples.iter().all(|s| s.is_finite()),
            "{fixture}: non-finite sample in decode"
        );
    }

    // The stereo .opus must split into two channels (Telegram/MediaRecorder
    // mono fixtures collapse to one).
    let bytes = std::fs::read(format!("{OPUS_DIR}/speech.opus")).expect("read fixture");
    let channels = gigastt::inference::audio::decode_audio_bytes_shared_channels(bytes.into())
        .expect("stereo .opus must decode");
    assert_eq!(channels.len(), 2, "stereo .opus must yield 2 channels");
    assert!(channels[0].len() >= 60_000);
}

#[ignore]
#[tokio::test]
async fn test_transcribe_opus_files() {
    let (port, shutdown) = common::start_server(&common::model_dir()).await;

    // OGG/Opus from a 16 kHz source, Telegram-voice-style 48 kHz mono, and a
    // stereo .opus: all decode via the opus-rs fallback and must produce a
    // sensible transcript.
    for (fixture, case) in [
        ("speech_16k.ogg", "OGG/Opus 16kHz"),
        ("speech_telegram.ogg", "Telegram-voice OGG/Opus"),
        ("speech.opus", "stereo .opus"),
    ] {
        let bytes = std::fs::read(format!("{OPUS_DIR}/{fixture}")).expect("read fixture");
        let (status, text) = post_transcribe(port, "", bytes).await;
        let body: serde_json::Value = serde_json::from_str(&text).expect("expected JSON body");
