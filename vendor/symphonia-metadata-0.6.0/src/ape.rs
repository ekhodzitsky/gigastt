    ...
    fn read<B: ReadBytes>(reader: &mut B) -> Result<ApeHeader> {
        ...
        let (size, has_footer, has_header, is_header) = match version {
            ...
            ApeVersion::V2 => {
                ...
                // Local overflow-guard patch (pending upstream): `size` is an
                // unbounded u32 read from untrusted input, so a crafted APEv2
                // header can make `size + 32` overflow and panic with "attempt
                // to add with overflow". Saturate instead of overflowing.
